...
const fetchFavouriteSongs = async () => {
  const response = await axios.get('/api/v1/favourite/get-user-songs');
  return response.data.favourite.songs;
};
...
export default function Page() {
  ...
  const { data: favouriteSongs = [], isLoading: isFavouriteLoading } = useQuery(
    'favouriteSongs',
    async () => {
      const songs = await fetchFavouriteSongs();
  ...
  const NoSongsMessage = () => (
    <div className="flex flex-col items-center justify-center h-64">
      <p className="text-xl font-semibold mb-2">No favourite song found</p>
      <p className="text-muted-foreground">Please add some songs to your favourite song</p>
    </div>
  );
    ...
    <ScrollArea className="h-full flex flex-col flex-1">
      <main className="flex-1 space-y-4 p-4 pt-6 md:p-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>Favourite</h1>
